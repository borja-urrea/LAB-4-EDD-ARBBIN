...
void insertTreeMap(TreeMap * tree, void* key, void * value) {
    ...
    TreeNode* nuevoDato = createTreeNode(key, value);
    nuevoDato -> parent = parent;

    if (parent == NULL){
        tree -> root = nuevoDato;
    } else if (tree -> lower_than(key, parent -> pair -> key)){
        parent -> left = nuevoDato;
    } else {
        parent -> right = nuevoDato;
    }
    tree -> current = nuevoDato;
}
...
void removeNode(TreeMap * tree, TreeNode* node) {
    ...
    if(node -> left == NULL && node -> right == NULL){
        ...
        if (padre == NULL){
            ...
            if (tree -> lower_than(node -> pair -> key, padre -> pair -> key)){
                padre -> left = NULL;
            } else{
                padre -> right = NULL;
            }
    ...
    //caso2
        ...
        node -> pair -> key = der-> pair -> key;
        node -> pair -> value = der -> pair -> value;
        removeNode(tree, der);
    }
    return;
}
